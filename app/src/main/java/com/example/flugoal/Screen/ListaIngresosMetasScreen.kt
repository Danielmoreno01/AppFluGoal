...
@Composable
fun ListaIngresosMetasScreen(navController: NavController, usuarioViewModel: UsuarioViewModel) {
    ...
    LaunchedEffect(usuarioId) {
        usuarioId?.let { movimientoViewModel.cargarIngresosMetasPorUsuario(it) }
    }
            ...
            LazyColumn {
                items(ingresosMovimientos) { movimiento ->
                    IngresosMItemStyled(
                        ...
                        fontFamily = robotoFont,
                        onEdit = {
                            movimiento.id?.let { navController.navigate("editar_ingreso_meta/$it") }
                        },
                        onDelete = {
                            movimiento.id?.let {
                                movimientoViewModel.eliminarMovimiento(it.toInt(),
                                    onSuccess = {
                                        usuarioId?.let { id ->
                                            movimientoViewModel.cargarIngresosMetasPorUsuario(id)
                                        }
                                    },
                                    onError = { errorMsg -> Log.e("ListaIngresosMetasScreen", "Error: $errorMsg") }
                                )
                            }
                        }
                    )
